...
Stand: 2026-09-25 · v3.0 (lock)

## Changelog

- **v3.0 (2026-09-25):** A2UI-Render (`Program.cs` `RenderA2uiHtml`) auf custom DOM-Renderer umgestellt (vanilla `createElement` + rekursiver Tree-Walk aus `<SITE.payload.messages>`, Top-Level = Top-Level-Messages ohne `children`-Referenz). `@a2ui/react@0.11.1` aus dem Importmap entfernt: offizielle Lib schluckt Martins `messages[].children = [id,id,id]`-Format im Zod-Schema silent (kein konsistenter Adapter möglich). Der v2.4-JS-Adapter ist obsolet. `RenderSchemaFormHtml`: `<meta name="viewport" content="width=device-width, initial-scale=1">` + mobile-responsive CSS-Block (`@media(max-width:600px)`, Inputs/Textareas/Selects/Buttons full-width + `min-height:44px` Touch-Targets, `form button[type='submit']` mit primary-button styling). Lock-Semantik-Footer.
